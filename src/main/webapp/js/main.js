...
var canvasmysql=document.getElementById("mysql").getContext("2d");
var canwidth=160;
var radius=70;
...
var canmysql=function(){
	var deg = 0;
    var mysqlT = function(deg){
        var r = deg*Math.PI/180;   
        canvasmysql.clearRect(0,0,canwidth,canwidth);   
        canvasmysql.beginPath();  
        canvasmysql.strokeStyle = "#031A3A"; 
        canvasmysql.lineWidth = 10; 
        canvasmysql.arc(canwidth/2,canwidth/2,radius,0-1/2*Math.PI,r-1/2*Math.PI,false); //
        canvasmysql.stroke();
    };
    var time = setInterval(function(){
    	var con=document.getElementsByTagName("section")[2];
